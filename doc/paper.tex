...
The last and final stage of the VQE finidind optimal paramters for our ansatz, which are the angles $\theta$ and $\phi$ corresponding to the rotations performed along the x- and y axes of the Bloch sphere. We could use the naive approach of incrementally iterating going through a large number of angles, and gain a precise approximation to the ground, however this will not suffice for larger many-body systems comprised of dozens of particles. Hundt proposes the use of gradient descent ([Hundt, p. 247]) as means of effectively computing angles for our ansatz. Since we 
